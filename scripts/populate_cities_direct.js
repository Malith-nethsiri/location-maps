#!/usr/bin/env node
/**
 * Direct City Population Script
 * Directly inserts essential Sri Lankan cities into Railway database
 */
...
async function populateCitiesDirectly() {
  try {
    console.log('🚀 Starting direct city population...');
    ...
    await query("DELETE FROM cities WHERE country = 'Sri Lanka'");
    ...
    // Verify insertion
    const result = await query("SELECT COUNT(*) as count FROM cities WHERE country = 'Sri Lanka'");
