...
CREATE INDEX IF NOT EXISTS idx_user_queries_geom ON user_queries USING GIST (geom);
...
-- Create triggers to automatically update geom columns from lat/lng
...
-- Apply triggers to all tables with coordinates
...
-- Create function to find nearest city
...
BEGIN
    ...
    SELECT
        ...
        c.population
    FROM cities c
    WHERE ST_DWithin(c.geom, ST_SetSRID(ST_MakePoint(target_lng, target_lat), 4326)::geography, max_distance_km * 1000)
